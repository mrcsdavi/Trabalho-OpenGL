...
class SombraClass:
    ...
    def desenharSombra(light_pos):
        chao_plano = [0.0, 1.0, 0.0, -0.001]  # plano y = 0
        sombra_mat = SombraClass.calcularShadowMatrix(chao_plano, light_pos)

        # Converta para tipo aceito por glMultMatrixf (coluna-major)
        ...
        glPushMatrix()
        glMultMatrixf(sombra_mat_gl)  # Aplica a matriz de sombra
        glTranslatef(0.0,0.5,0.0)       # mesma posição do cubo
        ObjetosClass.desenharCuboSemMaterial()
        glPopMatrix()
